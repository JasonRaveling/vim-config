...
" Last Change:      Thu, 22 Jun 2017 08:18:13 +0900
" Version:          4.37
"
" Description:
...
function GetShIndent()
  ...
  let line = s:BlankOrContinue(line, lnum)
  ...
  if cline =~# '^\s*\%(;;\|;&\|;;&\)\s*\%(#.*\)\=$'
    return s:CaseBreakIndent(ind) + s:sw()
  endif
  ...
  let ind = s:CurrentLineIndent(line, lnum, cline, ind)
...
function s:MorePrevLineIndent(pline, pnum, line, ind)
  ...
  elseif s:IsContinuLine(a:pline) && !s:IsContinuLine(line)
    ...
    if s:IsInSideCase(pline) || s:IsTailNoContinue(a:pline)
      let ind = a:ind
    endif
  endif
...
function s:CurrentLineIndent(line, lnum, cline, ind)
  let ind = a:ind
  ...
  if a:cline =~# '^\s*\%(then\|do\|else\|elif\|fi\|done\)\>[-=+.]\@!'
        \ || a:cline =~ '^\s*[})]'
    let ind = ind - s:sw()
  elseif a:cline =~# '^\s*[{(]\s*\%(#.*\)\=$' && s:IsTailAndOr(a:line)
...
function s:BlankOrContinue(line, lnum)
  if s:IsTailBackSlash(a:line) && a:lnum != v:lnum - 1
